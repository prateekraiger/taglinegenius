...
const LogoIcon = ({ className }: { className?: string }) => (
    <svg className={className || 'w-6 h-6'} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M12 2L14.5 9.5H22L16 14.5L18.5 22L12 17L5.5 22L8 14.5L2 9.5H9.5L12 2Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
);
    ...
    <header className="w-full sticky top-0 z-50 bg-light/80 dark:bg-dark/80 backdrop-blur-sm border-b border-primary-light/10 dark:border-primary-dark/10">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
                ...
                <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-secondary-light dark:text-secondary-dark">
                    <a href="#" className="hover:text-primary-light dark:hover:text-primary-dark transition-colors">Features</a>
                    <a href="#" className="hover:text-primary-light dark:hover:text-primary-dark transition-colors">Pricing</a>
                    <a href="#" className="hover:text-primary-light dark:hover:text-primary-dark transition-colors">About</a>
                </nav>
                <div className="flex items-center gap-2">
                    ...
                    <a href="#" className="hidden sm:inline-block bg-primary-light dark:bg-primary-dark text-primary-dark dark:text-primary-light font-semibold py-2 px-4 rounded-md hover:opacity-90 transition-opacity text-sm">
                        Get Started
                    </a>
                     <div className="md:hidden">
                        <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 rounded-full text-primary-light dark:text-primary-dark hover:bg-black/10 dark:hover:bg-white/10 transition-colors" aria-label="Toggle menu">
            ...
            {isMenuOpen && (
                 <nav className="md:hidden py-4 border-t border-primary-light/10 dark:border-primary-dark/10">
                    <a href="#" className="block py-2 text-base font-medium text-secondary-light dark:text-secondary-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors">Features</a>
                    <a href="#" className="block py-2 text-base font-medium text-secondary-light dark:text-secondary-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors">Pricing</a>
                    <a href="#" className="block py-2 text-base font-medium text-secondary-light dark:text-secondary-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors">About</a>
                    <a href="#" className="block mt-2 text-center bg-primary-light dark:bg-primary-dark text-primary-dark dark:text-primary-light font-semibold py-2 px-4 rounded-md hover:opacity-90 transition-opacity text-sm">
                        Get Started
                    </a>
                </nav>
            )}
...
const TaglineInputForm: React.FC<TaglineInputFormProps> = ({ userInput, setUserInput, handleSubmit, isLoading }) => (
...
const ResultCard: React.FC<ResultCardProps> = ({ tagline, isCopied, onCopy, animationIndex }) => {
    return (
        <div 
        ...
        >
            <p className="text-primary-light dark:text-primary-dark flex-1 pr-4">{tagline}</p>
            <button 
                onClick={onCopy}
                className={`p-2 rounded-full transition-all duration-200 ${isCopied ? 'bg-green-500/20 text-green-400' : 'text-secondary-dark hover:bg-black/10 dark:hover:bg-white/10'}`}
...
const GeneratedResultsList: React.FC<GeneratedResultsListProps> = ({ taglines, copiedIndex, setCopiedIndex }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 mt-10">
      <h2 className="text-lg font-semibold text-secondary-light dark:text-secondary-dark mb-4 text-center">Here are your AI-generated taglines!</h2>
        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                    <a href="https://x.com/mrpratik753" target="_blank" rel="noopener noreferrer" aria-label="Twitter" className="text-secondary-light dark:text-secondary-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors">
                        <TwitterIcon className="w-5 h-5"/>
                    </a>
                    <a href="https://github.com/prateekraiger/Riya" target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="text-secondary-light dark:text-secondary-dark hover:text-primary-light dark:hover:text-primary-dark transition-colors">
...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.length > 0 && (result[0].includes("Error") || result[0].includes("API") || result[0].includes("Sorry"))) {
        setError(result[0]);
        setGeneratedTaglines([]);
    } else {
  ...
  return (
    <div className="min-h-screen text-primary-light dark:text-primary-dark font-sans transition-colors duration-300 isolate flex flex-col">
        ...
        <main className="flex-grow w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 animate-fade-in-slow flex flex-col items-center pt-16 md:pt-24 pb-16">
            <div className="text-center mb-10 md:mb-12 px-4">
                ...
                <p className="text-lg md:text-xl text-secondary-light dark:text-secondary-dark max-w-2xl mx-auto">
                    Describe an epic pro play or ranked moment. Get AI headlines that sound like they're straight from a VCT broadcast. 🎤🏆
                </p>
            </div>
            <TaglineInputForm
                userInput={userInput}
            ...
            <GeneratedResultsList taglines={generatedTaglines} copiedIndex={copiedIndex} setCopiedIndex={setCopiedIndex} />
        </main>
